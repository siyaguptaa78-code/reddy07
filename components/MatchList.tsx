...
export const MatchList: React.FC<MatchListProps> = ({ category, iconUrl, title }) => {
  ...
  return (
    <div className="w-full flex flex-col gap-4 mt-6 select-none">
      {/* Category Header */}
      <div className="flex items-center gap-3 bg-[#0c1322] border border-[#9C27B0]/10 rounded-2xl p-4 shadow-sm">
      ...
      <div className="flex flex-col gap-3.5">
        {matches.length === 0 ? (
          <div className="bg-[#0c1322] border border-dashed border-[#9C27B0]/10 text-gray-400 text-center py-8 rounded-2xl">
          ...
          matches.map((match, idx) => (
            <div
              key={idx}
              className="bg-[#0c1322] hover:bg-[#10192b] border border-[#9C27B0]/10 hover:border-[#9C27B0]/20 transition-all duration-300 rounded-2xl p-4 md:p-5 flex flex-col md:flex-row md:items-center justify-between gap-4 shadow-[0_4px_20px_rgba(0,0,0,0.15)] group"
            >
              {/* Left Details */}
              ...
              <div className="flex items-center justify-between md:justify-end gap-5">
                ...
                {/* Date */}
                <div className="text-gray-400 font-bold text-xs md:text-sm bg-[#05080f] px-3 py-1.5 rounded-lg border border-[#9C27B0]/5">
                  📅 {match.date}
                </div>
